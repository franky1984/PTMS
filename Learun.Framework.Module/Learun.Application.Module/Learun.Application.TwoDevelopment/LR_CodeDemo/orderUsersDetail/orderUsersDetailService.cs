            ...
            try
            {
                ...
                else
                {
                    ...
                    if( checkError != 2 )
                    {
                        ...
                        //往关联细表中添加临时工与订单关联
                        this.BaseRepository().ExecuteBySql( "INSERT F_Base_TempWorkOrderUserDetail(f_id,F_TempWorkOrderId,f_userid,f_employerid,f_categoryid,F_WorkSubstitute,F_CreateUser,F_CheckBlack) VALUES(@f_id,@orderid,@userid,@employerid,@type,0,@createUser,0)", dp );
