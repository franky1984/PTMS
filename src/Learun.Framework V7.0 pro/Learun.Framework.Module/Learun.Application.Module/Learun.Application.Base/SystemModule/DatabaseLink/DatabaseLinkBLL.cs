...
namespace Learun.Application.Base.SystemModule
{
    ...
    public class DatabaseLinkBLL : DatabaseLinkIBLL
    {
        ...
        {
            databaseLinkService.InsertOrderDetail( identity, orderID, realName, categoryID, gender , employerID, mobile);
        }
        ...
        {
            return databaseLinkService.CheckBlack( identity );
        }
        ...
        /// <param name="sql">sql语句</param>
        /// <param name="dbParameter">参数</param>
        public void ExecuteBySql( string databaseLinkId, string sql, object dbParameter = null, string identity = "", string orderID = "", string realName = "", string categoryID = "", string gender = "", string employerID = "", string mobile = "")
        {
            try
            {
                DatabaseLinkEntity entity = GetEntity( databaseLinkId );
                databaseLinkService.ExecuteBySql( entity, sql, dbParameter, identity, orderID, realName, categoryID, gender, employerID, mobile );
            }
            catch( Exception ex )
            {
                if( ex is ExceptionEx )
        ...
        /// <param name="dbParameter">参数</param>
        /// <returns></returns>
        public DataTable GetOrdersTree(string databaseLinkId, string sql, object dbParameter = null)
        {
            try
            {
                if( sql == "select * from F_Base_TempWorkOrder" )
                {
                    sql = "select t.*,u.* from F_Base_TempWorkOrder";
                }

                UserInfo userInfo = LoginUserInfo.Get();
                //根据当前登录人的所属公司ID获取本公司下的所有活动列表
                sql += " t INNER JOIN LR_Base_User u ON t.f_createuser=u.f_userid  INNER JOIN LR_WF_ProcessInstance c ON t.F_OrderId=c.f_id AND c.F_IsFinished=1 AND (SELECT COUNT(*) FROM LR_WF_TaskHistory WHERE F_ProcessId=c.f_id AND F_Result = 2)=0 WHERE u.F_CompanyId='" + userInfo.companyId + "'";
